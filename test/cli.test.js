...
// ============================================================================
// End-to-End Tests (@e2e) — feature 04-static-site, §8.1 scenarios E1, E2
// ============================================================================

/** Deletes every .sdd/site key from a snapshot (frontier comparison). */
function withoutSite(files) {
  return Object.fromEntries(Object.entries(files).filter(([relative]) => !relative.startsWith('.sdd/site')));
}

/** Seeds the full mixed-state journey workspace: gitignore, chain, projections, index. */
async function seedSiteJourney(root) {
  await writeFiles(root, { '.gitignore': 'node_modules/\n' });
  await silently(() => init(ctx(root)));
  await silently(() => upsert(ctx(root, ['initiative'], { slug: 'layout-v3', title: 'Layout v3' })));
  await silently(() => upsert(ctx(root, ['feature'], { slug: '01-canonical-tree', title: 'Canonical tree', initiative: 'layout-v3' })));
  await silently(() => upsert(ctx(root, ['feature'], { slug: '02-static-base', title: 'Static base', initiative: 'layout-v3' })));
  await silently(() => upsert(ctx(root, ['spec'], { slug: '001-canonical-layout', title: 'Canonical layout', feature: '01-canonical-tree' })));
  await silently(() => upsert(ctx(root, ['spec'], { slug: '002-static-site', title: 'Static site', feature: '02-static-base' })));
  await silently(() => move(ctx(root, ['001'], { to: 'active' })));
  await silently(() => done(ctx(root, ['001'], { cascade: true })));
  await silently(() => move(ctx(root, ['002'], { to: 'active' })));
  await silently(() => render(ctx(root)));
}

test('[E1][INV-1..INV-4] full static-site journey from a blank workspace', async () => {
  const root = await makeWorkspace();
  try {
    await seedSiteJourney(root);

    // The site is built from the model alone and is fully navigable: every
    // link of every page resolves, relative to its own page, onto a page
    // that actually exists on disk.
    await silently(() => render(ctx(root, [], { site: true })));
    const sitePaths = new Set((await listPaths(root, '.sdd/site')).filter((entry) => entry.endsWith('.html')));
    assert.deepEqual([...sitePaths].sort(), [
      '.sdd/site/features/01-canonical-tree.html',
      '.sdd/site/features/02-static-base.html',
      '.sdd/site/index.html',
      '.sdd/site/initiatives/layout-v3.html',
      '.sdd/site/specs/001.html',
      '.sdd/site/specs/002.html',
    ]);

    for (const relative of sitePaths) {
      const html = await fsp.readFile(path.join(root, relative), 'utf8');
      const directory = path.posix.dirname(relative.slice('.sdd/'.length));
      for (const match of html.matchAll(/href="([^"]+)"/g)) {
        const resolved = path.posix.normalize(path.posix.join(directory, match[1]));
        assert.ok(
          [...sitePaths].some((entry) => entry.slice('.sdd/'.length) === resolved),
          `href "${match[1]}" in ${relative} resolves to "${resolved}" — not a generated page`,
        );
      }
    }

    // The dashboard → initiative → feature → spec chain is present, with a
    // consistent breadcrumb on the spec page.
    const specPage = await fsp.readFile(path.join(root, '.sdd/site/specs/001.html'), 'utf8');
    assert.match(specPage, /href="..\/index\.html"/);
    assert.match(specPage, /href="..\/initiatives\/layout-v3\.html"/);
    assert.match(specPage, /href="..\/features\/01-canonical-tree\.html"/);

    // validate stays green with site/ present; --check exits 0 without ever
    // mentioning site/ (its coverage is generated/ only).
    assert.equal(await silently(async () => {
      process.exitCode = 0;
      await validate(ctx(root));
      return process.exitCode;
    }), 0);
    let checkMentionsSite = false;
    const checkExit = await silently(async () => {
      const original = process.stdout.write;
      process.stdout.write = (chunk) => {
        if (String(chunk).includes('site/')) checkMentionsSite = true;
        return true;
      };
      try {
        process.exitCode = 0;
    ...
    assert.equal(checkExit, 0);
    assert.equal(checkMentionsSite, false);
    process.exitCode = 0;

    // The root .gitignore carries the entry exactly once.
...
test('[E2][INV-3] write-frontier inviolability and site idempotence', async () => {
  const root = await makeWorkspace();
  try {
    await seedSiteJourney(root);

    // First run: baseline (frontier = everything under .sdd/ except site/).
    await silently(() => render(ctx(root, [], { site: true })));
    const frontier = withoutSite(await snapshot(root, '.sdd'));
    const site = await snapshot(root, '.sdd/site');
    const gitignore = await fsp.readFile(path.join(root, '.gitignore'), 'utf8');

    // Second run: the workspace outside .sdd/site/ is bit-for-bit identical
    // after the execution, and the site tree is rebuilt byte-identical.
    await silently(() => render(ctx(root, [], { site: true })));
    assert.deepEqual(withoutSite(await snapshot(root, '.sdd')), frontier);
    assert.deepEqual(await snapshot(root, '.sdd/site'), site);
    assert.equal(await fsp.readFile(path.join(root, '.gitignore'), 'utf8'), gitignore);
  } finally {
    await cleanup(root);
  }
});
