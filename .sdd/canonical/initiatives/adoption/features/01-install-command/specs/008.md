...
* **Out of Scope (Strict Exclusions):**
  * Hôtes hors registre (extensible, hors scope v1), publication npm, connecteurs (`/sdd-setup`), gestion de versions.
...
|---|---|---|
| **Nominal** | `sdd install` (1+ hôtes détectés) | Plan → câblage → `sdd init` → récap + pointer `/sdd-setup`. |
