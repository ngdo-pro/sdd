...
* **User / System Impact:** Cutover dur vers un layout canonique sans état : le CLI ne lit **que** `.sdd/canonical/`. L'état vit exclusivement dans la métadonnée et l'index — `spec move` ne déplace plus aucun fichier. `knowledge/` regroupe décisions et domaines ; `spec init` ne pré-alloue rien. La conversion one-shot du workspace courant de ce repo est une tâche de la phase 3 ; l'industrialisation (`spec migrate`) est la feature `03-sdd-migration`.
* **In Scope (What is added / modified):**
  ...
  * `spec init` minimal : `.sdd/` + `canonical/` + `config.json`, rien d'autre.
...
│       ├── status.js / model.js / backend.js    [MOD] affichages préfixés .sdd/canonical/
...
└── backends/linear.js                           [MOD] affichage de chemin .sdd/canonical/
test/
...
|---|---|---|
| **Bootstrap** | `spec init` | Crée exactement `.sdd/`, `.sdd/canonical/` et `config.json`. Aucun autre répertoire. |
...
* **INV-1 · Aucun état de cycle de vie dans un chemin canonique**
  Tout chemin sous `.sdd/canonical/` dérive exclusivement de slugs/ids et de `relations` ; les répertoires `planned/`, `active/`, `archive/` n'y existent pas et `validate` le refuse. `spec move` ne change aucun chemin.
...
* **`index.json` déplacé:** tout lecteur externe pointant `.sdd/model/index.json` casse (agents, mirrors, scripts). La one-shot supprime l'ancien fichier ; balayer `status.js`, `model.js`, `backend.js`, `help.js`, `linear.js`, `config.js` dont les chaînes codées en dur mentionnent `.sdd/model/`.
