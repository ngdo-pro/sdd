...
* **User / System Impact:** Un nouveau skill `/sdd-setup` guide l'adoptant de zéro à un workspace configuré et validé : connecteurs activés, settings vérifiés contre les vraies sources, transport MCP résolu depuis les configs de l'hôte — le tout persisté par les commandes `sdd` (jamais d'écriture directe).
...
|---|---|---|
| **Nominal** | `/sdd-setup` sur repo sans `.sdd/` | Interview → découverte → validation → synthèse → dry-run → confirmation → `sdd init` validé. |
...
| **Workspace existant** | re-run `/sdd-setup` | `sdd status` lu d'abord ; ajustements proposés en merge (idempotent). |
...
- [x] **Phase 2: Wiring docs**
  - [x] `README.md` : section skills + mention `/sdd-setup` dans le flow d'adoption.
