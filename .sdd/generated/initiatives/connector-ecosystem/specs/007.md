...
* **INV-1 · Grammaire unifiée + dry-run réel** — `connectors enable|disable` passent par les mêmes helpers que `init` (subkeys namespacées, répétition → tableau, coercion) ; `--dry-run` n'écrit rien ; le contournement documenté dans `/setup` est retiré de `SKILL.md`.
...
* **Ne pas casser la parité `/setup` :** mettre à jour `skills/setup/SKILL.md` (la note « pas de dry-run sur connectors enable » devient fausse) — édition knowledge/skill autorisée (fichier authored).
