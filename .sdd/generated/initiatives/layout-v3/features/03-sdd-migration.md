...
* **INV-4:** le succès est strict : `spec validate` exit 0 **et** `spec render --check` exit 0 sur le workspace migré — sinon `.sdd/` est marqué en échec, `.sdd/` est conservé et l'erreur remontée ; le rollback hors git est explicitement Out-of-Scope.
* **INV-5:** si `.sdd/` et `.sdd/` coexistent après tentative, toute mutation du CLI est refusée (exit 1) avec l'instruction de relancer `spec migrate` ; en lecture, le CLI opère sur `.sdd/` uniquement.
