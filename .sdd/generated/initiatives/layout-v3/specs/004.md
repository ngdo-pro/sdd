# Spec: 004 - Site statique de consommation

## Metadata
* **Domain:** `.sdd/knowledge/domains/workspace-management/`
* **Change Type:** `New Capability`
* **Complexity:** `Medium`
* **Feature:** `04-static-site`
...
- [ ] **Phase 1: Fondations & contrats de chemins**
  - [ ] `src/core/paths.js` : `SITE_DIRNAME`, `siteRoot()`, `ALLOWED_ROOT_ENTRIES` + `'site'` (docstring : tolérée, jamais exigée, jamais pré-allouée).
  - [ ] `src/render/site.js` : `escapeHtml`, constante CSS unique, `buildSiteModel(artifacts)` (lecture pure), `renderBodyHtml(body)` (convertisseur minimal, gherkin inclus).
  - [ ] Tests @unit U1–U4 dans `test/static-site.test.js` ; adaptation U2 de `test/generated-namespace.test.js` (5 entrées).

- [ ] **Phase 2: Pages & CLI**
  - [ ] `src/render/site.js` : `renderSite(cwd, artifacts)` — purge + écriture (chrome, dashboard, pages initiative/feature/spec, fil d'Ariane, liens croisés relatifs, états vides français).
  - [ ] `src/cli/commands/render.js` : branche `--site` (UsageError conflit, garde coexistence, modèle vide, table, `--json`, exit 0) ; `src/cli/main.js` (flag) ; `src/cli/help.js`.
  - [ ] `src/cli/commands/init.js` : entrée `.gitignore` idempotente (existe ET absent).
  - [ ] Tests @component C1–C8 + @integration I1–I2 dans `test/static-site.test.js`.

- [ ] **Phase 3: E2E, documentation & quality gates**
  - [ ] Tests @e2e E1–E2 dans `test/cli.test.js`.
  - [ ] `README.md` : section CLI (`spec render --site`) + règle « init ajoute `.sdd/site/` au `.gitignore`, render ne touche jamais » + arbre `.sdd/` avec `site/` en entrée optionnelle.
