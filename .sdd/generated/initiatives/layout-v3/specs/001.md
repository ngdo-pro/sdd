...
* **User / System Impact:** Cutover dur vers un layout canonique sans état : le CLI ne lit **que** `.sdd/canonical/`. L'état vit exclusivement dans la métadonnée et l'index — `sdd move` ne déplace plus aucun fichier. `knowledge/` regroupe décisions et domaines ; `sdd init` ne pré-alloue rien. La conversion one-shot du workspace courant de ce repo est une tâche de la phase 3 ; l'industrialisation (`sdd migrate`) est la feature `03-sdd-migration`.
* **In Scope (What is added / modified):**
  ...
  * `sdd move` sans relocation de fichier ; `sdd upsert spec` exige `--feature` ; `sdd link` dérive `relations.initiative` et relaie la relocation aux descendants.
  * `sdd init` minimal : `.sdd/` + `canonical/` + `config.json`, rien d'autre.
  * `sdd validate` : règles `canonical-layout` et `spec-id-uniqueness` ; ids de spec à 4 chiffres tolérés au-delà de 999.
...
* **Out of Scope (Strict Exclusions):**
  * Le namespace `generated/` (feature `02-generated-namespace`) — les projections interim restent aux chemins v2 (`.sdd/specs/<state>/`, `.sdd/initiatives/…`, `.sdd/vision.md`), le rendu est un watchout §6.
  * Le renommage `.sdd/` (feature `03-sdd-migration`) — la racine reste `.sdd/`.
  * `sdd migrate` industrialisé, migration des workspaces v2 tiers, commande de rename de slug (PDR-001 : slug immuable), mirroring Linear (inchangé).
...
sequenceDiagram
    ...
    U->>CLI: sdd move 042 --to archived
    ...
    alt spec sans --feature / parent introuvable
        ...
        CLI->>CLI: sdd validate → finding(s), exit 1
    end
```
...
│   └── config.js                                [MOD] chaîne doc « .sdd/canonical/ » (cosmétique)
├── model/
...
│   ├── index.js                                 [MOD] index v3 écrit dans .sdd/canonical/index.json, préfixes .sdd/canonical/
...
│   ├── help.js                                  [MOD] texte d'aide « .sdd/canonical/ »
│   └── commands/
│       ├── init.js                              [MOD] layout minimal : .sdd/ + canonical/ + config.json, rien d'autre
...
│       ├── status.js / graph.js / connector.js    [MOD] affichages préfixés .sdd/canonical/
...
└── connectors/linear.js                           [MOD] affichage de chemin .sdd/canonical/
test/
...
export const canonicalRoot = (cwd) => path.join(cwd, SPECS_DIRNAME, CANONICAL_DIRNAME);   // .sdd/canonical/
export const knowledgeRoot = (cwd) => path.join(cwd, SPECS_DIRNAME, KNOWLEDGE_DIRNAME);   // .sdd/knowledge/
...
// src/model/store.js
loadModel(cwd);                                      // walk récursif de .sdd/canonical/ (vision, initiatives/**)
...
* **`canonical/index.json` (version 3)** : déplacé de `.sdd/model/index.json` vers `.sdd/canonical/index.json`. Entrées inchangées sauf préfixes : `meta`/`body` → `.sdd/canonical/…`, `projection` → `.sdd/…` (interim v2, jusqu'à la feature 02).
* **Arbre canonique livré** (racine `.sdd/` jusqu'à la feature 03) :
...
|---|---|---|
| **Bootstrap** | `sdd init` | Crée exactement `.sdd/`, `.sdd/canonical/` et `config.json`. Aucun autre répertoire. |
| **Création spec** | `sdd upsert spec --slug 042-x --feature <ref>` | Résout la feature → dérive `relations.initiative` → écrit à l'emplacement canonique définitif. Sans `--feature` : `UsageError` (chemin non dérivable), rien d'écrit. |
| **Transition** | `sdd move <ref> --to <state>` | Mute `state` + `updatedAt` dans la métadonnée in situ, régénère index + projections. `dry-run` sans écriture. |
| **Achevement** | `sdd done <ref> [--cascade]` | Persist + archive (métadonnées seules) ; cascade sans relocation ; `--undo` retourne à `active`. |
| **Re-parentage** | `sdd link <spec> --feature <ref>` | Écrit `relations.{feature, initiative}` (initiative dérivée du parent) ; relocate les fichiers de la spec si le parent a changé. |
| **Re-parentage feature** | `sdd link <feat> --initiative <slug>` | Reloge la feature **et** toutes ses specs descendantes (ordre topologique). Le slug est immuable (INV-6). |
...
* **INV-1 · Aucun état de cycle de vie dans un chemin canonique**
  Tout chemin sous `.sdd/canonical/` dérive exclusivement de slugs/ids et de `relations` ; les répertoires `planned/`, `active/`, `archive/` n'y existent pas et `validate` le refuse. `sdd move` ne change aucun chemin.
  ↳ *Covered by:* [`canonical-layout.test.js`](#appendix-file-index), [`cli.test.js`](#appendix-file-index)

* **INV-2 · `sdd move` ne déplace aucun fichier**
...
* **INV-3 · Répertoires tous créés à la volée ; `sdd init` ne pré-alloue rien**
  `standardLayout` = [`.sdd/`, `.sdd/canonical/`] ; `saveArtifact` crée les répertoires manquants (`ensureDir`) à chaque écriture, y compris `knowledge/**` côté agents. Après `init`, l'arbre contient exactement 2 répertoires + `config.json`.
  ↳ *Covered by:* [`canonical-layout.test.js`](#appendix-file-index)

* **INV-4 · `knowledge/` regroupe `decisions/{architecture,product}/` et `domains/` ; `decisions/` disparaît du premier niveau**
  `knowledgeRoot` est la référence unique ; la migration one-shot (phase 3) déplace `.sdd/decisions/**` vers `.sdd/knowledge/decisions/**` et les skills écrivains (`new-adr`, `new-pdr`) sont repointés.
  ↳ *Covered by:* [`canonical-layout.test.js`](#appendix-file-index)
...
* **INV-7 · Allocation d'id séquentielle, jamais réutilisée ; 4 chiffres tolérés au-delà de 999**
  `parseSpecSlug` accepte `\d{3,4}` ; l'allocation est séquentielle (max existant + 1, calculé via `sdd list --kind spec --json`) et un id libéré n'est jamais réalloué (pas de commande delete exposée).
  ↳ *Covered by:* [`canonical-layout.test.js`](#appendix-file-index)
...
* **Double `vision.md` interim:** jusqu'à la feature 02, `.sdd/canonical/vision.md` (authored) coexiste avec `.sdd/vision.md` (projection). Ne pas « corriger » les projections interim vers `generated/` — c'est le scope de la feature 02 ; documenter la convention dans le README du repo.
* **Scanners legacy gelés:** `src/core/artifact.js`, `src/migrate/import.js`, `src/migrate/parse.js` encodent encore les états en chemins et une regex `\d{3}` (incompatible 4 chiffres). Ne pas les brancher sur `canonical/` ; rework complet en feature 03 (`sdd migrate`). `sdd import` est obsolète après cutover.
* **`index.json` déplacé:** tout lecteur externe pointant `.sdd/model/index.json` casse (agents, mirrors, scripts). La one-shot supprime l'ancien fichier ; balayer `status.js`, `graph.js`, `connector.js`, `help.js`, `linear.js`, `config.js` dont les chaînes codées en dur mentionnent `.sdd/model/`.
* **Relocation en cascade au re-parentage:** `sdd link feature --initiative <autre>` doit reloger la feature PUIS ses specs descendantes (ordre topologique, `previous`-based) ; sinon les specs restent orphelines à l'ancien chemin et le prochain `loadModel` les perd. Interdire l'opération si une spec descendante manque (validate d'abord).
...
* **Sequencing du cutover:** implémenter et valider le code sur des workspaces temporaires AVANT la one-shot du repo ; sinon `sdd validate`/`render` du repo tournent sur un layout à moitié converti.
...
- [x] **Phase 2: Cœur logique & interfaces CLI**
  ...
  - [x] `src/model/index.js` : index v3 dans `.sdd/canonical/index.json` ; `src/cli/context.js` sans threading `initiativeState`.
  - [x] Commandes : `init` (minimal), `upsert` (spec exige `--feature`, dérive `initiative`), `move`/`done` (zéro mv), `link` (dérive + relocation descendants), `validate` (règles `canonical-layout`, `spec-id-uniqueness`), chaînes d'affichage (`status`, `model`, `connector`, `help`, `linear.js`, `config.js`).
...
- [x] **Phase 3: Cutover one-shot, E2E & quality gates**
  - [x] Migration one-shot du workspace de ce repo : `model/**` → `canonical/**` (re-nesting features/specs, renommage `specs/<id>`), `decisions/**` → `knowledge/decisions/**`, suppression des répertoires d'état vides et de l'ancien `.sdd/model/`, régénération index + projections.
  ...
  - [x] Exécuter 100 % des gates : `npm test` (93+ tests verts), `sdd validate` exit 0, `sdd render --check` sans drift.
...
Feature: Layout canonique v3 — état en métadonnées, arbre canonical/
  ...
  Scenario: [INV-3] standardLayout ne retourne que la racine et canonical/
    ...
    Then le tableau vaut exactement [".sdd/", ".sdd/canonical/"] — aucun répertoire d'état, ni model/, ni decisions/, ni knowledge/
  ...
  @unit
  Scenario: [INV-1] buildIndex v3 préfixe meta/body par .sdd/canonical/ et écrit dans canonical/index.json
    Given trois artefacts hydratés depuis canonical/
    When buildIndex(artifacts) puis writeIndex
    Then index.json version 3 vit sous .sdd/canonical/index.json, meta/body préfixés ".sdd/canonical/", projection préfixée ".sdd/"
  ...
  @component
  Scenario: [INV-2] sdd move planifie la mutation sans relocation
  ...
  Scenario: [INV-1] upsert spec sans --feature est rejeté avant toute écriture
    ...
    When `sdd upsert spec --slug 042-x` (sans --feature)
  ...
  Scenario: [INV-1][INV-5] link dérive relations.initiative et reloge la spec au changement de parent
    ...
    When `sdd link 042 --feature 01-other` (feature sous initiatives/i2/features/b/)
  ...
  Scenario: [INV-1][INV-6] re-parenter une feature reloge la feature et toutes ses specs descendantes
    ...
    When `sdd link 01-a --initiative i2`
  ...
  Scenario: [INV-1] projections interim v2 et index canonical coexistent sans conflit
    ...
    Then la projection reste à .sdd/specs/active/… (interim v2), l'index à .sdd/canonical/index.json, et render --check ne signale aucun drift
  ...
  Scenario: [INV-3] init produit l'arbre minimal exact
    ...
    When `sdd init`
    Then l'arbre contient exactement .sdd/config.json et .sdd/canonical/ — aucun répertoire d'état, ni model/, ni knowledge/

  @integration
  Scenario: [INV-1][INV-2][INV-3] cycle complet CLI sans aucun déplacement de fichier
    Given un workspace après `sdd init`
    When upsert initiative → feature → spec, puis `sdd move` sur chacun
  ...
  Scenario: [INV-5][INV-7] ids 3 et 4 chiffres coexistent et les refs par id nu résolvent
    ...
    When `sdd list --kind spec --json` puis résolution par "042" et "1042"
  ...
  Scenario: [INV-1..INV-7] parcours utilisateur complet depuis un workspace vierge
    ...
    When `sdd init`, upsert initiative "layout-v3", re-upsert avec un titre modifié, upsert feature "01-canonical-tree", upsert spec "001-canonical-layout" --feature, `sdd move` jusqu'à archived, `sdd done --cascade`, `sdd validate`
  ...
  Scenario: [INV-2] idempotence du move : ré-exécution et dry-run sans effet de bord
    ...
    When `sdd move 001 --to archived` (déjà archived), puis `--dry-run`, puis un second move réel
...
| `connector.js` | `src/cli/commands/connectors.js` |
