...
* **In Scope (What is added / modified):**
  ...
  * Erratum de traçabilité de la spec 002 (§3.1) : ajout de `test/model-store.test.js` à l'inventaire — appliqué via `sdd upsert` (rule 7), pas à la main.
...
* **Out of Scope (Strict Exclusions):**
  ...
  * Le mirroring Linear et les connectors distants : les refs `remote` sont préservées telles quelles, aucune resynchronisation n'est déclenchée par la migration.
...
sequenceDiagram
    ...
    participant M as sdd migrate (moteur src/migrate/migrate.js)
    ...
    M-->>U: workspace migré — un second sdd migrate est un no-op

    alt layout non identifiable (markdown seul, sans index)
        M-->>U: erreur orientée sdd import (markdown) ou sdd init — rien d'écrit
    else gate en échec
        M->>D: marker .sdd/.migration-failed.json écrit
        M-->>U: exit 1 — .sdd/ conservé, relancer sdd migrate (reprise de zéro)
    else coexistence .sdd/ + .sdd/ (toute autre commande mutatrice)
        M-->>U: mutation refusée (exit 1) — instruction : relancer sdd migrate
    end
```
...
└── cli/
    ...
    └── commands/
        ├── migrate.js                           [NEW] sdd migrate [--dry-run] [--json] — plan, exécution, gate, suppression source
        ...
        ├── import.js                            [MOD] assertNoCoexistence ; message d'orientation sdd migrate
        ├── init.js / graph.js / status.js / connector.js   [MOD] garde coexistence (init --force, model --write) ; chaînes d'affichage .sdd/
test/
...
└── extensions/README.md, extensions/linear/{README.md,extension.json}, extensions/_TEMPLATE/connector.js   [MOD] descripteur d'artefact v3 + chemins
```
...
//   unsupported  : tout le reste (markdown seul → orienter sdd import ; vide → orienter sdd init)
...
export function convertConfig(raw);            // → { config, warnings } — clés top-level inconnues et connectors
...
//   refuse (UsageError, exit 2) si .sdd/canonical/index.json ou .sdd/model/index.json existe → orienter sdd migrate
...
* **`config.json` v3 (arbitrage 4)** : `convertConfig` conserve les clés connues (`version`, `sourceOfTruth`, `projections`, `connectors`), retire toute autre clé top-level et tout connector `filesystem` avec warning listé, force `version: 3`, écrit `.sdd/config.json`. `schemas/config.schema.json` : `"version": { "const": 3 }`, descriptions repointées `.sdd/`.
...
|---|---|---|
| **Prévisualisation** | `sdd migrate --dry-run` | Détection + plan complet (artefacts par kind, divergences, réécritures de tokens, warnings config, suppressions planifiées) ; se termine par `(dry-run: nothing was written)` ; exit 0 même si des divergences bloquantes existent (elles sont listées comme abort planifié). |
| **Workspace v2 pur** | `sdd migrate` | Relations dérivées du contenu → `.sdd/` reconstruit → gate → `.sdd/` supprimé. |
| **Workspace v3 sous `.sdd/`** | `sdd migrate` | Reconstruction directe depuis les métadonnées → gate → `.sdd/` supprimé. |
| **Déjà migré** | `sdd migrate` | No-op : « already migrated », exit 0, rien d'écrit. |
| **Coexistence** | `sdd migrate` (reprise) | `.sdd/` partiel ou en échec est **effacé intégralement** (listé) puis reconstruit de zéro (INV-1). |
| **Coexistence** | toute autre commande mutatrice (`upsert`, `move`, `done`, `link`, `render` en écriture, `import`, `init --force`, `model --write`, `sync`, `connector enable/disable`) | Refusée (exit 1) : « `.sdd/` et `.sdd/` coexistent — relancez `sdd migrate` ». Les lectures (`status`, `list`, `validate`, `render --check`/`--dry-run`, `model` en lecture) opèrent sur `.sdd/` uniquement. |
| **Gate en échec** | `sdd validate` findings ≠ 0 **ou** drift `render --check` ≠ 0 | `.sdd/.migration-failed.json` écrit (stage + message), `.sdd/` conservé, exit 1, erreur remontée. |
| **Markdown seul** | `sdd migrate` | `unsupported` : erreur orientant vers `sdd import` (conversion complète markdown) ou `sdd init` (workspace vierge). |
| **Markdown seul, post-bascule** | `sdd import [--dry-run]` | Conversion complète via le finish partagé : import des markdown v2 sous `.sdd/canonical/`, tokens réécrits, gate strict, suppression de `.sdd/` au succès. Refuse (exit 2, `UsageError`) si un modèle JSON existe → orienter `sdd migrate`. |

Sortie type d'un `sdd migrate --dry-run` :
...
| Extensions | `extensions/README.md` (dont descripteur d'artefact lignes 66-67 → `model: { directory: 'initiatives/<init>/features/<f>', meta: '…/specs/<id>.json', body: '…/specs/<id>.md' }`, `projection: 'generated/initiatives/<init>/specs/<id>.md'`), `extensions/linear/README.md`, `extensions/linear/extension.json`, `extensions/_TEMPLATE/connector.js` | Chemins modèle + config → `.sdd/…` |

Le workspace de ce repo (`.sdd/**` : corps canoniques, fields, `knowledge/**`) n'est **pas** repointé à la main : sa conversion passe par la one-shot `sdd migrate` (phase 3), qui applique la réécriture mécanique des tokens (INV-3). Les chaînes d'affichage du CLI (`init.js`, `status.js`, `graph.js`, `connector.js`, `help.js`, `connectors/linear.js` docstring `.remote-map`) sont mises à jour dans le même scope — les chemins fonctionnels dérivent déjà de `SPECS_DIRNAME`.

> *Clean Omission Note : aucune variable d'environnement, conteneur ou pipeline modifié — les gates CI appellent déjà `sdd validate` et `sdd render --check`.*
...
* **INV-1 · Reconstruction intégrale, jamais de merge — idempotence par construction**
  `sdd migrate` reconstruit **toujours** `.sdd/` de zéro depuis la source ; un `.sdd/` existant (partiel, en échec, obsolète) est effacé avant reconstruction. Un second `sdd migrate` sur un workspace déjà migré est un no-op. Aucun chemin de merge ni de diff de répertoire.
...
* **INV-4 · Succès strict — sinon marker d'échec, source conservée, erreur remontée**
  La migration ne déclare le succès que si `sdd validate` produit 0 finding **et** `sdd render --check` 0 drift sur le workspace migré (gate in-process, mêmes moteurs que les commandes). Sinon : `.sdd/.migration-failed.json` écrit (stage + message), `.sdd/` conservé intégralement, exit 1. Le rollback hors git est explicitement Out-of-Scope.
...
* **INV-5 · Coexistence `.sdd/` + `.sdd/` : mutations refusées, lectures sur `.sdd/`**
  Tant que les deux racines coexistent, toute commande mutatrice est refusée (exit 1) avec l'instruction de relancer `sdd migrate` ; en lecture, le CLI opère exclusivement sur `.sdd/` (dérivations `SPECS_DIRNAME`).
...
* **INV-6 · `--dry-run` sur toutes les écritures de la migration**
  `sdd migrate --dry-run` et `sdd import --dry-run` calculent le plan complet (divergences, réécritures, warnings, suppressions) sans aucune écriture ; l'arborescence est bit-à-bit inchangée et la sortie se termine par `(dry-run: nothing was written)`.
...
* **Horizon MANAGED réduit :** après retrait du sweep v2, un `.sdd/` contenant des entrées `specs/`, `initiatives/` ou `vision.md` parasites n'est plus purgé au render — `validate` (règle `root-layout`) les refuse et la reprise passe par `sdd migrate`. Ne pas réintroduire le sweep « par sécurité ».
...
* **Mirrors silencieux :** la migration ne déclenche aucune opération distante ; les refs `remote` sont copiées telles quelles. Un connector activé n'est ni interrogé ni resynchronisé — `sdd sync` reste l'outil de réconciliation (Out-of-Scope ici).
...
* **`sdd import` vs `sdd migrate` :** `import` refuse dès qu'un modèle JSON existe (`.sdd/canonical/index.json` ou `.sdd/model/index.json`) — sinon il reconstruirait depuis les positions des markdown (violation d'INV-2). Ne pas ré-autoriser ce cas.
...
- [x] **Phase 2: Moteur de migration, import re-owné & garde de coexistence**
  ...
  - [x] `src/cli/commands/migrate.js` [NEW] + enregistrement `main.js`/`help.js` ; garde `assertNoCoexistence` sur `upsert`, `move`, `done`, `link`, `render` (écriture), `import`, `init --force`, `model --write`, `sync`, `connector`.
...
- [x] **Phase 3: Cutover one-shot, repointage doc-wide, E2E & quality gates**
  - [x] One-shot du workspace courant : commit propre → `sdd migrate --dry-run` → vérifier le plan (divergences, réécritures) → `sdd migrate` → `git status` (`.sdd/**` ajoutés, `.sdd/` supprimé) → re-gates.
  - [x] Erratum spec 002 (§3.1) : `sdd upsert spec --slug 002-generated-namespace --feature 02-generated-namespace --from <corps>` avec `test/model-store.test.js` ajouté à l'inventaire.
  ...
  - [x] Chaînes d'affichage du CLI (`init.js`, `status.js`, `graph.js`, `connector.js`, `help.js`, `connectors/linear.js`) repointées `.sdd/`.
...
Feature: Migration .sdd/ — cutover de racine, reconstruction et coexistence
  ...
  Scenario: [U3][INV-3] convertConfig produit un config v3 épuré avec warnings
    Given un config v2 avec une clé top-level inconnue "legacyDirs" et un connector "filesystem"
    When convertConfig(raw) est appelée
    Then le config sortant vaut version 3, sans "legacyDirs" ni connector filesystem, projections et connectors conservés
    And warnings contient exactement un message pour la clé inconnue et un pour le connector retiré
  ...
  Scenario: [C5][INV-5] Coexistence : mutations refusées, lectures sur .sdd/
    ...
    When sdd upsert, sdd move, sdd render (écriture), sdd import, sdd init --force, sdd graph --write sont tentés
    Then chacun sort 1 avec l'instruction de relancer sdd migrate et rien n'est écrit
    And sdd status, sdd list, sdd validate, sdd render --check et sdd render --dry-run s'exécutent en lisant .sdd/ uniquement

  @component
  Scenario: [C6][INV-6] migrate --dry-run n'écrit rien
    Given un workspace v3 sous .sdd/ avec divergences et tokens à réécrire
    When sdd migrate --dry-run est exécuté
  ...
  Scenario: [C7][INV-1][INV-3] Second migrate = no-op et préservation vérifiée champ par champ
    ...
    When sdd migrate est relancé puis sdd status --json sur chaque artefact
  ...
  Scenario: [I1][INV-3][INV-4] Conversion complète v3-canonical avec config v3 et tokens réécrits
    ...
    When sdd migrate --dry-run puis sdd migrate, puis sdd validate et sdd render --check
  ...
  @integration
  Scenario: [I2][INV-6][arbitrage 8] sdd import convertit un workspace markdown-only post-bascule
    Given un workspace avec uniquement des markdown v2 sous .sdd/ (aucun index JSON)
    When sdd import --dry-run puis sdd import
    ...
    And un sdd import sur un workspace doté d'un modèle JSON est refusé avec orientation vers sdd migrate
  ...
  Scenario: [E1][INV-1..INV-6] Parcours complet : workspace v2 pur vers workspace .sdd/ opérationnel
    ...
    When sdd migrate --dry-run, sdd migrate, sdd upsert spec, sdd move, sdd validate, sdd render --check
    ...
    And un second sdd migrate est un no-op et validate + render --check sortent 0
  ...
  Scenario: [E2][INV-4][INV-5] Échec de gate, verrou de coexistence, reprise de zéro
    ...
    When une mutation (sdd upsert) est tentée puis sdd migrate relancé après correction de la source
...
| `graph-cmd.js` | `src/cli/commands/graph.js` |
| `status.js` | `src/cli/commands/status.js` |
| `connector-cmd.js` | `src/cli/commands/connectors.js` |
...
| `template-connector.js` | `extensions/_TEMPLATE/connector.js` |
