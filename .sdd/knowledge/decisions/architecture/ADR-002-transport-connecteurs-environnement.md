...
* **Status:** Accepted — **implémentée par la spec `005-linear-mcp`** (feature `03-linear-mcp` ; résolution du transport par le skill `/setup`, spec `006-setup-skill`)
* **Date:** 2026-09-13
* **Impact:** `Architecture`
...
* **Option A — Statu quo (clé API dans le framework) :** transport direct, simple à tester, mais le framework détient et manipule des secrets ; chaque connecteur réimplémente client + auth ; contradiction avec le futur skill `/setup` qui valide via MCP.
...
* **Repli explicite :** sans transport MCP résolu dans les settings, le connecteur est inopérant — `sdd validate` le signale (`connector-settings`), `sync` échoue avec un message invitant à `/setup` ; jamais de fallback implicite vers une auth directe.
