...
* **Contrats gérés dans ce domaine :**
  ...
  * Contrat d'amorçage d'hôte agentique : `sdd install [--host auto|opencode|claude|agents] [--init/--no-init] [--dry-run] [--undo]` — détection par marqueurs (`opencode.json`, `.claude/`, `.agents/`) → plan pur → câblage idempotent journalisé (merge structurel `skills.paths` de `opencode.json`, symlinks `.claude/`/`.agents/` pointant le package en cours d'exécution) → `sdd init` ; `--undo` retire exactement le journal `.sdd/.install-journal.json`, unique source d'undo (feature 01-install-command, spec 008 ; cf. §2, §3, §4).
...
| `sdd install` | `[--host auto\|opencode\|claude\|agents]`, `--init/--no-init`, `--dry-run`, `--undo` | Phase order **détection → câblage → `sdd init`** ; `--host` explicite gagne sur la détection ; plusieurs hôtes détectés sans `--host` : multi-select TTY (`@clack/prompts` en import dynamique, annulation ⇒ « Install cancelled ») ou `UsageError` hors TTY ; câblage idempotent — `opencode.json` : merge structurel de `"skills": { "paths": ["node_modules/shodo/skills"] }` (mutation in place, clés/ordre préservés, valeur ajoutée une seule fois, ENOENT ⇒ forme minimale) et symlinks `dir` `.claude/{skills,agents}/shodo` (resp. `.agents/…`) → pkgRoot résolu du module exécuté (`import.meta.url`, jamais `process.cwd()`) ; cible étrangère préexistante ⇒ warn + skip (jamais d'écrasement) ; symlink shodo périmé (source journalisée) ⇒ repointé vers le pkg courant ; `symlink()` EPERM (Windows) ⇒ copie récursive + warning ; journal fusionné (`kind:target`, première occurrence gagne) écrit **après** init dans un `finally` ; aucun hôte détecté ⇒ init seul + instructions manuelles (templates non câblés — les agents les lisent du package au runtime) ; récap + pointer `/setup` ; `--dry-run` : plan affiché (hôtes, entrées, init), zéro écriture ; `--undo` : retire exactement les entrées journalisées (symlinks + valeur `opencode.json`, nettoyage des feuilles `paths`/`skills` vides), journal supprimé au succès | `UsageError` exit 2 — `--host` inconnu (registre cité), plusieurs hôtes sans `--host` hors TTY (liste `id — evidence`), `--undo` sans journal (« Nothing to undo — no install journal found at .sdd/.install-journal.json »), annulation TTY ; `ConfigError` exit 1 — `opencode.json` illisible ou non-objet : rien d'écrasé |
...
* **Contrat du journal d'installation (`.sdd/.install-journal.json` — feature 01-install-command, spec 008) :** tableau JSON d'entrées posées par `sdd install`, écrit après `sdd init` dans un `finally` (le câblage reste réversible même si init échoue), toléré par `root-layout` (fichier caché, cf. `.migration-failed.json`) ; consommé et supprimé par un `--undo` réussi — **unique source d'undo** (jamais de suppression « à la devinette » sans journal) :

```json
[
  { "kind": "opencode-path", "target": "node_modules/shodo/skills", "source": "/…/node_modules/shodo/skills", "created": "2026-09-14T10:00:00.000Z" },
  { "kind": "symlink", "target": ".claude/skills/shodo", "source": "/…/node_modules/shodo/skills", "created": "2026-09-14T10:00:00.000Z" },
  { "kind": "symlink", "target": ".claude/agents/shodo", "source": "/…/node_modules/shodo/agents", "created": "2026-09-14T10:00:00.000Z" }
]
```

  * `kind` ∈ `symlink | opencode-path` ; `target` = chemin repo-relatif (emplacement du symlink ou valeur `skills.paths`) ; `source` = chemin absolu dans le package exécuté (pkgRoot) ; `created` = stamp ISO de la pose.
  * Fusion par clé `kind:target` : première occurrence gagne (le `created` d'origine est conservé), la `source` périmée est rafraîchie au re-install (le journal reflète toujours le pkg exécuté).
  * `readJournal` retourne `null` si le fichier est absent, illisible ou non-tableau — `--undo` échoue alors proprement en `UsageError`.
  * À l'`--undo`, un symlink ne pointant plus la `source` journalisée est laissé intact (warning) et une entrée de fallback copie (EPERM Windows) est laissée en place avec warning — indistinguable d'un contenu utilisateur.
...
| `sdd install` sur une cible préexistante étrangère (symlink/répertoire ≠ package shodo) | Pas une erreur : warning + skip (`"…" already exists and is not owned by shodo — skipped`), l'install continue, exit 0 | Rien d'écrasé (INV-2) — retirer/renommer la cible étrangère puis relancer `sdd install` pour la câbler |
| `opencode.json` illisible ou non-objet lors du merge | `ConfigError`, exit 1 — « fix it before running `sdd install` (nothing was overwritten) » | Rien d'écrasé — corriger `opencode.json` puis relancer |
