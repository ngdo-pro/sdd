...
* **Contrats gérés dans ce domaine :**
  ...
  * Contrat de manifest connecteur : `extensions/<id>/extension.json` — `settings` (défauts), `settingTypes` (`string | boolean | number | object`), `requiredSettings` (auditées par `validate` pour les connecteurs activés).
...
| `sdd move <ref>` | `--to planned\|active\|archived`, `--dry-run` | Mute `state` in situ ; régénère index + projections **au même chemin** (zéro mv, zéro rename) | `TransitionError` si illégale |
...
*Contrat de manifest connecteur (`extensions/<id>/extension.json` — additif, rétrocompatible) :*

```json
{
  "settings": { "teamKey": "", "createOnMove": false, "labels": { "spec": "spec" } },
  "settingTypes": { "teamKey": "string", "createOnMove": "boolean", "labels": "object" },
  "requiredSettings": ["teamKey"]
}
```
...
| Violation de règles (`validate`) | findings + exit 1 (dont `root-layout` pour toute entrée parasite de la racine `.sdd/`, et `connector-settings` pour un connecteur activé incomplet ou mal typé) | Lecture seule, aucun effet de bord |
