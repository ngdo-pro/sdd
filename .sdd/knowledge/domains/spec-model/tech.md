...
> [!NOTE]
> **Mission :** Architecture du CLI `spec` et du modèle canonique sans état : lecture/écriture de `.sdd/canonical/` (état en métadonnées), index v3, projections markdown sous `.sdd/generated/` (namespace exclusif, régénéré et purgé par le CLI), migration de workspaces hérités par reconstruction intégrale (`sdd migrate`) et garde-fous d'intégrité (drift de projections, racine exhaustive, coexistence verrouillée).
...
| **Mirrors** | Projection distante optionnelle ; refs stockées dans la métadonnée `remote` (préservées telles quelles par la migration — jamais resynchronisées par elle) | `src/connectors/linear.js` (désactivé par défaut), `src/connectors/registry.js` (catalogue + fabriques) |

> [!IMPORTANT]
> **Frontière de Domaine :** les namespaces `generated/` (feature `02-generated-namespace`), la migration de workspaces `sdd migrate` (feature `03-sdd-migration` — cutover de racine `.specs/` → `.sdd/` effectué, workspace de ce repo basculé en one-shot) et l'init déclarative des connecteurs (feature `01-declarative-init` — `sdd init --connector`, interview TTY, règle `connector-settings`, première dépendance runtime `@clack/prompts` cf. [ADR-001](../../decisions/architecture/ADR-001-dependance-runtime-clack-prompts.md)) sont livrés. Reste scellé à la feature sœur : l'entrée racine optionnelle `site/` (feature `04`, extension de `ALLOWED_ROOT_ENTRIES`) et toute validation réseau/MCP du connecteur Linear (feature `02-setup-skill`).
...
> *Traçabilité : les invariants `INV-1`…`INV-4` de la feature `02-generated-namespace` (spec 002, §5) correspondent aux `INV-8`…`INV-11` du domaine, les invariants `INV-1`…`INV-6` de la feature `03-sdd-migration` (spec 003, §5) correspondent aux `INV-12`…`INV-17` du domaine, et les invariants `INV-1`…`INV-5` de la feature `01-declarative-init` (spec 004, §5) correspondent aux `INV-18`…`INV-22` du domaine (numérotation locale à la feature).*
