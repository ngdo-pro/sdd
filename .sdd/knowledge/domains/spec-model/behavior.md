...
> **Acteurs & Personas :** Utilisateur humain (lead/dev qui pilote le process), Agent (orchestrateurs spécialisés : `/sdd-initiative`, `/sdd-feature`, `/spec`, `/sdd-sync-knowledge`…), Agent amorceur (`/sdd-setup` — voie d'adoption unique du framework, protocole v2 `skills/sdd-setup/SKILL.md`).
...
* **Ce qui relève de ce domaine (In Scope) :**
  ...
  * Adoption guidée du framework : l'agent `/sdd-setup` est l'unique voie d'adoption (protocole v2, phases conditionnelles et confirmées) — état des lieux systématique (lecture seule), migration legacy conditionnelle (`sdd migrate`), câblage + init (`sdd install`), connecteur optionnel (interview, découverte read-only du transport MCP de l'hôte, validation sémantique via MCP, application CLI-only via `sdd connectors enable … --dry-run`), récapitulatif → `/sdd-vision` — tout passant par le CLI (cf. `PAR-WM-07`).
  ...
  * Notice de mise à jour post-run : après chaque commande `sdd`, une notice discrète sur stderr signale une version plus récente du package (best-effort, cache 24 h stampé `installedVersion` et invalidé au changement de version du binaire, opt-outs flag/env) — l'ordre de sortie, la propreté de stdout et l'exit code des commandes ne sont jamais affectés (cf. `RULE-WM-29`…`RULE-WM-33`).
...
flowchart TD
    ...
    Start -.->|adoption du framework — /sdd-setup voie unique| Setup["Agent /sdd-setup<br>état des lieux → migration legacy → câblage + init → connecteur ? → récap"]
    ...
    Notice -.->|cache frais < 24h, version installée inchangée| NoFetch[aucun fetch, aucun affichage]
    Notice -.->|cache < 24h mais upgrade — version ≠ cache, ou cache legacy| Refresh["cache invalidé — check frais<br>(un latest null périmé ne survit plus à l'upgrade)"]
```
...
| `PAR-WM-07` | Adopter le framework (`/sdd-setup` — protocole v2) | Agent amorceur (`/sdd-setup`) | Commande `/sdd-setup` (ou demande d'adopter le framework / de configurer un miroir) | Repo adoptant opérationnel : legacy migré le cas échéant, workspace câblé + initialisé, connecteur activé et validé si voulu — persisté exclusivement par le CLI |
| `PAR-WM-08` | Amorcer un repo adoptant (`sdd install`) | Utilisateur / Agent | `sdd install [--host <id>] [--dry-run] [--undo] [--no-init]` | Hôtes câblés (skills/agents → package installé), workspace initialisé, récap + pointer `/sdd-setup` — ou plan affiché en `--dry-run`, câblages retirés en `--undo` |
...
* **Acteur :** Agent orchestrateur (skills `/sdd-initiative`, `/sdd-feature`, `/spec`).
...
* **Acteur :** Agent orchestrateur (`/sdd-sync-knowledge`), après quality gates verts.
...
### `PAR-WM-07` : Adopter le framework (`/sdd-setup` — protocole v2)
* **Acteur :** Agent amorceur — le protocole de référence est le skill `skills/sdd-setup/SKILL.md` ; `/sdd-setup` est l'unique voie d'adoption du framework (l'`sdd install` du `PAR-WM-08` et le `sdd migrate` du `PAR-WM-06` sont les primitives CLI qu'il orchestre) ; cette fiche résume son parcours observable côté adoptant.
...
* **Déroulement nominal :** 1. la commande s'exécute normalement — la sortie utile (stdout) et l'exit code sont intégralement préservés ; 2. opt-out ? flag `--no-update-check` ou env `SDD_NO_UPDATE_CHECK` non vide ⇒ no-op ; 3. cache de moins de 24 h (mémoire puis `.sdd/.update-check.json`) **et** stampé de la version courante (`installedVersion`) ⇒ aucun fetch, aucun affichage ; 4. cache expiré **ou invalidé** (mismatch de version, legacy sans stamp, stamp malformé) ⇒ un seul fetch du registre npm (`registry.npmjs.org/@ngdo-pro/sdd/latest`, timeout 1,5 s) et comparaison semver strict ; 5. version distante plus récente que l'installée ⇒ une ligne stderr après la sortie : `· update available: vX.Y.Z (installed: vA.B.C) — npm i -g @ngdo-pro/sdd` ; 6. le cache est rafraîchi dans tous les cas (échec réseau et égalité de version inclus) — chaque écriture (mémoire + dotfile) embarque `installedVersion`, la version du binaire au moment de l'écriture.
...
* **Variantes :** *Offline / timeout / registre 404 / JSON ou semver malformé :* silencieux — aucune notice, aucune erreur ; `lastCheck` est quand même rafraîchi (les sessions offline ne re-sondent pas à chaque commande). *Version égale ou inférieure :* cache rafraîchi, aucune notice. *Upgrade du binaire (cache < 24 h mais `installedVersion` ≠ version courante) :* le cache est invalidé malgré sa fraîcheur — un check frais part immédiatement et le cache est réécrit, stampé de la nouvelle version (le dotfile n'est jamais supprimé, juste remplacé) ; un `latest: null` hérité d'une fenêtre de réplication npm ou d'une session offline sous la version précédente ne survit plus à l'upgrade — la notice réapparaît au premier run si le registre annonce plus récent (bug de production corrigé par la spec 011). *Cache legacy (dotfile sans `installedVersion`) ou stamp malformé (non-string) :* idem — check frais, réécriture stampée. *Notice déjà affichée il y a < 24 h :* le cache répond sans fetch (`cached`) et rien n'est ré-affiché — au plus un affichage par 24 h. *Workspace sans `.sdd/` :* cache mémoire seul (durée de vie = session), aucun dotfile écrit — la notice fonctionne normalement. *`--dry-run` :* la notice s'affiche comme pour toute commande et le cache dotfile s'écrit (il est hors graphe). *CI :* idem normal — le cache partagé évite que des invocations courtes se sondent en boucle.
...
* **`RULE-WM-17` (Inopérant sans transport résolu) :** sans `settings.mcp` (absent ou vide), `sync`/`move` n'opèrent aucun mirroring — erreur par artefact « no MCP transport configured — run /sdd-setup », aucun `remoteRef` écrit ; `sdd validate` signale le connecteur incomplet (exit 1) ; jamais de repli implicite vers une auth directe.
...
* **`RULE-WM-26` (Install offline) :** `sdd install` n'ouvre aucun réseau, ne demande aucune clé — tout passe par `node:fs` ; `sdd init` conserve sa sémantique (local par défaut, connecteurs via `--connector`/`/sdd-setup`).
...
| `/sdd-setup` (phase 1) sur un repo où `.specs/` et un `.sdd/` valide coexistent | Arbitrage présenté — contenu, dates et états des deux côtés + verdict CLI verbatim : `sdd migrate` efface et reconstruit `.sdd/` depuis la source legacy, et toute autre écriture reste verrouillée entre-temps (`RULE-WM-09`) ; jamais de migration silencieuse | L'humain arbitre : confirmer `sdd migrate` (reconstruction de zéro, git en filet) ou repartir d'un état propre (restaurer via git puis retirer la racine legacy), puis relancer `/sdd-setup` |
...
| `sdd connectors enable <id>` laissant les réglages obligatoires incomplets | Activation enregistrée + hint du manifest affiché (ex. « Run /sdd-setup… ») ; sans `hint`, message générique listant les réglages manquants | Suivre le hint (skill `/sdd-setup`) ou renseigner les réglages manquants avec les mêmes flags |
| `sdd sync` (ou `move` d'un artefact lié) où tous les miroirs activés échouent | Warnings par artefact/connecteur puis synthèse `all enabled mirrors failed (n/n)`, **exit 1** — jamais une exception ; le rapport détaillé reste affiché | Corriger le transport MCP (`/sdd-setup`) ou le serveur distant, puis relancer `sdd sync` / `sdd move` |
...
| `sdd sync --create` (ou `move` d'un artefact lié) avec linear activé sans `settings.mcp` | Erreur par artefact « no MCP transport configured — run /sdd-setup » (warning) ; aucun `remoteRef` écrit ; la commande se termine normalement | Lancer le skill `/sdd-setup`, ou `sdd connectors enable linear --linear.mcp.command=… --linear.mcp.args=…` — puis `sdd validate` (exit 0 attendu) |
...
| Première configuration d'un connecteur miroir (phase 3 de `/sdd-setup`) | Parcours guidé (`PAR-WM-07`, protocole v2) : interview → découverte MCP read-only → validation → dry-run (`connectors enable … --dry-run`) → application CLI-only | Re-run `/sdd-setup` pour des ajustements (mode merge, idempotent) |
...
| Notice de mise à jour — cache frais (< 24 h) et version installée inchangée | Aucun fetch, aucun affichage — même si une version plus récente est déjà connue du cache (au plus 1 affichage/24 h) ; comportement 009 strictement inchangé | Supprimer `.sdd/.update-check.json` pour forcer un re-check (affichage au prochain run si version plus récente) |
| Notice de mise à jour — cache < 24 h mais périmé par le binaire : `installedVersion` ≠ version courante (upgrade), cache legacy sans le stamp, ou stamp non-string | Cache ignoré à la lecture (jamais supprimé) — un check frais part malgré la fraîcheur ; un `latest: null` périmé hérité de la version précédente n'est plus servi (bug de production corrigé — le cache ne survit plus à un upgrade) ; le cache est réécrit frais, stampé de la version courante, dès le check | Rien à faire — le premier run après l'upgrade re-sonde et répare le cache |
