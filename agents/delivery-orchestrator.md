...
|---|---|---|---|
| **1. Spec Framing** | `agents/spec-writer.md` | `skills/sdd-spec/SKILL.md` | `.sdd/canonical/initiatives/<initiative>/features/<feature>/specs/<id>.{json,md}` |
| **2. Build Code** | `agents/implementer.md` | `skills/sdd-build-spec/SKILL.md` | Compiled code & executed migrations (spec moved to `active/` via `sdd move`) |
| **3. Quality Gates** | `agents/qa-tester.md` | `skills/sdd-test-spec/SKILL.md` | 100% passing tests (Unit, Component, E2E) |
| **4. Clean-Room Audit** | `agents/reviewer.md` | Clean-Room Protocol | Audit report (Spec vs Git Diff) |
| **5. Capitalize & Sync** | `agents/knowledge-orchestrator.md` | `skills/sdd-sync-knowledge/SKILL.md` | Updated `.sdd/knowledge/` & archived chain via `sdd done [id] --cascade` |
...
6. **Closure & Hand-off to Knowledge Track:**
   - Upon formal approval (`APPROVED`):
     * Hand off execution to `knowledge-orchestrator` (`/sdd-sync-knowledge [id]`).
