...
> [!NOTE]
> **Mission :** Architecture du CLI `spec` et du modèle canonique sans état : lecture/écriture de `.specs/canonical/` (état en métadonnées), index v3, projections markdown sous `.specs/generated/` (namespace exclusif, régénéré et purgé par le CLI) et garde-fous d'intégrité (drift de projections, racine exhaustive).
...
flowchart LR
    ...
    CLI -->|régénération à chaque écriture| Render["🖨 Projections<br><i>(src/render/* — namespace .specs/generated/)</i>"]
...
| **Projections** | Rendu markdown sous `.specs/generated/` (aplati au niveau initiative, trié par id) ; purge des orphelins + sweep v2 ; prévisualisation `--dry-run` ; garde anti-drift `render --check` limitée à `generated/` | `src/render/{projections,markdown}.js`, `src/model/layout.js` (`projectionRelativePath`) |
...
|---|---|---|
| **Expose à** | `02-generated-namespace` | Namespace `generated/` exclusif (**livré**) : toutes les projections markdown vivent sous `.specs/generated/` ; champ `projection` de `index.json` reciblé (format v3 inchangé, pas de version 4) |
...
| **`INV-8`** | **Projections exclusivement sous `.specs/generated/`, racine `.specs/` exhaustive** : `config.json`, `canonical/`, `generated/`, `knowledge/` — toute autre entrée est une violation | `validate` règle `root-layout` ; `render --check` (couverture `generated/` seul) |
...
|---|---|---|
| **Projections v2 aux chemins avec dirs d'état** : `.specs/specs/<state>/`, `.specs/initiatives/<state>/<init>/…`, `.specs/vision.md` | Chemins encodant l'état, renommage des projections au `move` | **TOMBÉE (basculée par 002)** : projections sous `.specs/generated/` — features et specs aplaties au niveau initiative, triées par id, zéro répertoire d'état, un `move` régénère au même chemin |
| **Double `vision.md`** : `.specs/vision.md` (projection) coexiste avec `.specs/canonical/vision.md` (authored) | Deux visions indiscernables à la racine | **TOMBÉE (basculée par 002)** : une seule vision, à `generated/vision.md` |
| **Préfixes `index.json`** : `meta`/`body` = `.specs/canonical/…`, `projection` = `.specs/…` (chemins v2) | Champ projection pointant l'arbre v2 | **TOMBÉE (basculée par 002)** : `projection` = `.specs/generated/…` — format v3 inchangé, pas de version 4 |
