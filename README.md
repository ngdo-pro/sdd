...
.specs/
├── config.json
├── canonical/                                   # source of truth (stateless)
...
> **Interim conventions (until the `02-generated-namespace` feature ships):**
> the generated markdown still lives at the v2 state-encoded paths — `.specs/specs/<state>/`, `.specs/initiatives/<state>/…`, `.specs/vision.md` — which is why `.specs/canonical/vision.md` (authored) coexists with `.specs/vision.md` (projection). Feature projections default to the `planned` initiative segment. Do not "fix" these paths by hand; they are regenerated by the CLI.
