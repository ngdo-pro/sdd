...
```text
   ...
   spec CLI         │  <artifact>.json  metadata         │
           ...
           ┌───────────────────────┼──────────────────────┐
           ▼                       ▼                      ▼
    .specs/generated/**      Linear issues          GitHub Issues
    (markdown docs)          (mirror)               (planned)
```

* **Canonical:** `.specs/canonical/` — one JSON metadata file + one markdown body file per artifact. The layout is **stateless**: lifecycle lives in the `state` metadata field and the index, never in a path (`spec move` relocates no file).
* **Knowledge:** `.specs/knowledge/` — authored content outside the graph: `decisions/{architecture,product}/` (ADR/PDR) and `domains/`.
* **Generated:** the markdown documents under `.specs/generated/` (roadmaps, checkboxes, `Status:` headers) and remote issues.
...
```bash
spec init                        # bootstrap .specs/canonical/ + config
...
spec render [--check] [--dry-run] # regenerate .specs/generated/ (--check = CI drift guard)
